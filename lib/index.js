...
export const Config = z.object({
  /** Default Obsidian vault to validate when the tool is called without a `vault` arg. */
  defaultVault: z.string().default('D:/Obsidian/MyKnowledgeBase'),
})
  ...
  if (tools) {
    ...
    const tool = defineTool({
      ...
      parameters: {
        vault: {
          type: 'string',
          description: `Absolute path to the Obsidian vault to validate. Defaults to the plugin config defaultVault ("${defaultVault}").`,
        },
      },
      output: {
        schema: {
